<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column prop="name" label="分類名稱" :show-overflow-tooltip="true" width="300"></el-table-column>
      <el-table-column prop="icon" label="圖示" align="center" width="100">
        <template slot-scope="scope">
          <svg-icon v-if="scope.row.icon" :icon-class="scope.row.icon"/>
        </template>
      </el-table-column>
      <el-table-column prop="cardCount" label="卡片數" width="60"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleRemove(row) {
      ...
      this.$modal.confirm('將移除到歷史分類，是否確認移除選取的資料？').then(() => {
        return this.$http.patch('/api/v1/category/valid/false', ids);
      }).then(() => {
        this.getList();
        this.$modal.msgSuccess("已移除到歷史分類");
      });
    },
  }
}
